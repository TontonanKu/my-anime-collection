...
async function fetchForMovie(movie) {
  try {
    ...
    console.error(`Failed to fetch for ${movie.title}:`, e);
  }
  return null;
}
...
async function run() {
  for (const movie of movies) {
    if (movie.characters) {
      console.log(`Skipping ${movie.title}, already has characters`);
      continue;
    }
    console.log(`Fetching characters for: ${movie.title}`);
    const chars = await fetchForMovie(movie);
    if (chars) {
      movie.characters = chars;
      console.log(`-> Found ${chars.length} characters (source: ${chars[0]?.voiceActor?.language === 'Mandarin' ? 'MAL' : 'AniList/MAL'})`);
    } else {
      console.log(`-> Found 0 characters`);
    }
    
    // Save incrementally
    fs.writeFileSync(moviesFile, JSON.stringify(movies, null, 2));
    
    // Strict delay to avoid Jikan rate limit globally
